...
def Modulation(bits, mu):
    ...
    return 0.7071 * (2 * bit_r[:, 0] - 1) + 0.7071j * (2 * bit_r[:, 1] - 1)
...
def get_pilot_bits(P):
    P = P * 2
    Pilot_file_name = 'data/Pilot_' + str(P)
    ...
    pilot1 = pilotValue[0:P:2] 
    pilot2 = pilotValue[1:P:2]

    def _deModulation(Q):
        Qr = np.real(Q)
        Qi = np.imag(Q)
        bits = np.zeros([Qr.shape[0], 2])
        bits[:, 0] = Qr > 0
        bits[:, 1] = Qi > 0
        return bits.reshape([-1])  # This is just for QAM modulation

    def _get_bits(pilot, carriers):
        OFDM_data = np.zeros(K, dtype=complex)
        OFDM_data[carriers] = pilot  # allocate the pilot subcarriers
        return _deModulation(OFDM_data)
    bits0 = _get_bits(pilot1, pilotCarriers1)
    bits1 = _get_bits(pilot2, pilotCarriers2)
    
    return np.concatenate((bits0, bits1), 0)

def get_pilot_carriers(P):
    P = P * 2
    Pilot_file_name = 'data/Pilot_' + str(P)
    if os.path.isfile(Pilot_file_name):
        bits = np.loadtxt(Pilot_file_name, delimiter=',')
    ...
    def _get_bits(pilot, carriers):
        ...
        # OFDM_f = DFT(OFDM_withCP)
        return OFDM_data
    bits0 = _get_bits(pilot1, pilotCarriers1)
    bits1 = _get_bits(pilot2, pilotCarriers2)
    
    return np.concatenate((bits0, bits1), 0), np.concatenate((pilotCarriers1, pilotCarriers2), 0), np.concatenate((pilot1, pilot1, pilot2, pilot2), 0)
...
def ofdm_simulate(codeword, channelResponse, SNRdb, mu, CP_flag, K, P, CP, pilotValue, pilotCarriers, dataCarriers,
    ...
    if Clipping_Flag:
        OFDM_withCP_cordword = Clipping(
            OFDM_withCP_cordword, CR)  # add clipping
    ...
    BB = np.concatenate((np.real(OFDM_RX_noCP_codeword),
